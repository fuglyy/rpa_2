...
if __name__ == "__main__":
    def main_makPal_291_proc():
        try:
            ...
            log_process(window_log=True,block_text='Присвоить значение переменной')
            #gs[2)Lvz];bQqHa!`hqL
            notification = ''.join([str(x) for x in ['Анализ активности студентов Q1 завершён.', ('/'), (''), f"Всего записей: {total_records}", (''), f"Незавершённых модулей: {incomplete_modules}", (''), f"Дней с низким баллом тестов: {low_quiz_score_days}", (''), f"Коротких сессий: {short_sessions}", (''), (''), f"Всего завершённых уроков: {total_lessons}", f"Всего выполненных заданий: {total_assignments_done}", (''), (''), 'Отчёт сохранён в DesignCourses_Result.xlsx (лист Analysis_Q1).']])
            log_process(block_text='Присвоить значение переменной')
            #}!JE_T#-7v-VD.S*tc$n
